...
class Events extends Component {
  state = {status: 'initial', selected: ''}

  onClickItem = item =>
    this.setState({status: item.registrationStatus, selected: item.id})
  ...
  render() {
    ...
    return (
      <div className="mainContainer">
        <div className="eventItemSection">
          <h1 className="eventsheading">Events</h1>
          <ul className="eventItemContainer">
            {eventsList.map(each => (
              <EventItem
                selected={selected}
                item={each}
